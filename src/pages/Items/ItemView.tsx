...
// Define supported item status values locally
...
const ItemView: React.FC = () => {
  ...
  }, [item, withConfirmation, withLoading, deleteItem, navigate]);
  ...
  }, [item, updateStatus, success, showError]);
  ...
  const handleDownloadPDF = useCallback(() => {
    warning('PDF download will be implemented soon.');
    setTimeout(() => window.print(), 500);
  }, [warning]);
  ...
  const handleArchive = useCallback(async () => {
    ...
    try {
      ...
      showError('Failed to archive item');
    }
  }, [item, success, showError]);
  ...
  const getHeaderDropdownItems = (): ThreeDotDropdownItem[] => {
    ...
    items.push({ label: 'Edit', icon: <Edit className="h-4 w-4" />, onClick: handleEdit });
    ...
    items.push({ label: 'Share', icon: <Share2 className="h-4 w-4" />, onClick: handleShare });
    items.push({ label: 'Print', icon: <Printer className="h-4 w-4" />, onClick: handlePrint });
    items.push({ label: 'Download PDF', icon: <FileDown className="h-4 w-4" />, onClick: handleDownloadPDF });
    items.push({ label: 'Duplicate', icon: <Copy className="h-4 w-4" />, onClick: handleDuplicate });
    items.push({ label: 'Archive', icon: <Archive className="h-4 w-4" />, onClick: handleArchive });
    items.push({ label: 'Delete', icon: <Trash2 className="h-4 w-4" />, onClick: handleDelete, danger: true });
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4 mb-6 print:mb-4">
          ...
          <div className="print:hidden">
            <ThreeDotDropdown items={getHeaderDropdownItems()} position="right" />
          </div>
        </div>
