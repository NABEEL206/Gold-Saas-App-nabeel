  ...
  const columns: TableColumn<PaymentMade>[] = [
    {
      ...
      render: (item) => (
        <div>
          <p className="text-sm font-medium text-gray-900">{item.paymentNumber}</p>
          <p className="text-xs text-gray-500">{new Date(item.paymentDate).toLocaleDateString()}</p>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      header: 'Reference',
      render: (item) => (
        <span className="text-sm text-gray-600">{item.referenceNumber || '-'}</span>
      ),
    },
    {
      key: 'status',
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Manage your vendor payments</p>
        </div>
      ...
      <ReusableTable
        ...
        emptyIcon={<CreditCard className="h-12 w-12 text-gray-300" />}
